...
message_complete_df_long <- message_complete_df %>% 
 ...
 mutate(topic = case_when(motive == "Information" & target == "ET" & topic == 1 ~ "Existence",
  ...
  mutate(pre_ukr = case_when(Datum < as.Date("2022-02-24") ~ "before",
         ...
         pre_hamas = factor(pre_ukr, levels = c("before", "after")))
...
Special focus on the War/ecocrisis theme, but also the affiliation topic. War/ecocrisis are overarching categories and not within the hierachical structure as seen in the "figures" folder. 

Dates with no data points do not necessarily have 0 messages, but can also have 0 non-transcribed messages. 

It is unclear which time frame we should use (7, 14, 21 days prior/past?)
Date of occurrence is "past" 
